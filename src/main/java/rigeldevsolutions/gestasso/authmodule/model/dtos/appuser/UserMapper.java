...
public abstract class UserMapper
{
    ...
    @Mapping(target = "grade", expression = "java(dto.getGradeCode() == null || dto.getGradeCode().trim().equals(\"\") ? null : new rigeldevsolutions.gestasso.grademodule.model.entities.Grade(dto.getGradeCode()))")
    @Mapping(target = "active", expression = "java(false)")
    @Mapping(target = "notBlocked", expression = "java(true)")
    public abstract AppUser mapToAdherant(CreateMembreDTO dto);
}