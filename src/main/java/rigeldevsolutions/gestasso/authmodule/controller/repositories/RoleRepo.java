...
public interface RoleRepo extends JpaRepository<AppRole, String>
{
    ...
    @Query("select new rigeldevsolutions.gestasso.sharedmodule.dtos.SelectOption(r.roleCode, r.roleName) from AppRole r")
    List<SelectOption> getAllAsSelectOptions();

    @Query("select new rigeldevsolutions.gestasso.sharedmodule.dtos.SelectOption(r.roleCode, r.roleName) from AppRole r where r.roleCode in ?1")
