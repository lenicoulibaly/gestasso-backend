...
import rigeldevsolutions.gestasso.authmodule.model.dtos.appprivilege.PrvByTypeDTO;
...
public interface PrvRepo extends JpaRepository<AppPrivilege, String>
{
    ...
    @Query("""
    select new rigeldevsolutions.gestasso.authmodule.model.dtos.appprivilege.ReadPrvDTO
    (
    ...
    @Query("""
            select new rigeldevsolutions.gestasso.authmodule.model.dtos.appprivilege.PrvByTypeDTO
            (
                p.prvType.name, p.prvType.uniqueCode, 
                (select new rigeldevsolutions.gestasso.authmodule.model.dtos.appprivilege.ReadPrvDTO(p1.privilegeCode, p1.privilegeCode, p1.privilegeName, p1.prvType.name) from AppPrivilege p1 where p1.prvType.uniqueCode = ?1)
            ) 
            from AppPrivilege p where p.prvType.uniqueCode = ?1
             """)
    ...
    @Query("select new rigeldevsolutions.gestasso.sharedmodule.dtos.SelectOption(p.privilegeCode, p.privilegeName, p.prvType.name) from AppPrivilege p where p.prvType.uniqueCode in ?1")
    List<SelectOption> getPrivilegesByTypes(List<String> typeCodes);

    @Query("select new rigeldevsolutions.gestasso.sharedmodule.dtos.SelectOption(p.privilegeCode, p.privilegeName, p.prvType.name) from AppPrivilege p")
