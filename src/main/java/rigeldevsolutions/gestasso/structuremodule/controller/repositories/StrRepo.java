package rigeldevsolutions.gestasso.structuremodule.controller.repositories;

import rigeldevsolutions.gestasso.sharedmodule.dtos.SelectOption;
import rigeldevsolutions.gestasso.structuremodule.model.dtos.ChangeAnchorDTO;
import rigeldevsolutions.gestasso.structuremodule.model.dtos.ReadStrDTO;
import rigeldevsolutions.gestasso.structuremodule.model.dtos.ChangeAnchorDTO;
import rigeldevsolutions.gestasso.structuremodule.model.dtos.ReadStrDTO;
...
public interface StrRepo extends JpaRepository<Structure, Long>
{
    ...
    @Query("select s from Structure s where (locate(function('getStrCode', ?1), concat(s.strCode, '/')) = 1 or s.strId = ?1) and (coalesce(upper(function('unaccent', s.strName) ), '') like upper(concat('%', coalesce(?2, ''), '%')) or coalesce(upper(function('unaccent',s.strSigle)), '') like upper(concat('%', coalesce(?2, ''), '%')) or coalesce(upper(function('unaccent',s.strTel)), '') like upper(concat('%', coalesce(?2, ''), '%')) or coalesce(upper(function('unaccent', s.strAddress)), '') like upper(concat('%', coalesce(?2, ''), '%')) or coalesce(upper(function('unaccent', s.situationGeo)), '') like upper(concat('%', coalesce(?2, ''), '%')) or upper(function('unaccent',s.strType.name)) like upper(concat('%', coalesce(?2, ''), '%'))) and s.status = 'ACTIVE' order by s.strName")
    ...
    @Query("select s from Structure s where (locate(function('getStrCode', ?1), concat(s.strCode, '/')) = 1 or s.strId = ?1) and (coalesce(upper(function('unaccent', s.strName) ), '') like upper(concat('%', coalesce(?2, ''), '%')) or coalesce(upper(function('unaccent',s.strSigle)), '') like upper(concat('%', coalesce(?2, ''), '%')) or coalesce(upper(function('unaccent',s.strTel)), '') like upper(concat('%', coalesce(?2, ''), '%')) or coalesce(upper(function('unaccent', s.strAddress)), '') like upper(concat('%', coalesce(?2, ''), '%')) or coalesce(upper(function('unaccent', s.situationGeo)), '') like upper(concat('%', coalesce(?2, ''), '%')) or upper(function('unaccent',s.strType.name)) like upper(concat('%', coalesce(?2, ''), '%'))) and s.status = ?3 order by s.strName")
    Page<Structure> searchStr(Long strId, String key, PersStatus status, Pageable pageable);

    @Query("select new rigeldevsolutions.gestasso.structuremodule.model.dtos.ReadStrDTO(s.strId, s.strName, s.strSigle, function('get_hierarchy_sigles',s.strId)) from Structure s where (locate(function('getStrCode', ?1), concat(s.strCode, '/')) = 1 or s.strId = ?1) and (coalesce(upper(function('unaccent', s.strName) ), '') like upper(concat('%', coalesce(?2, ''), '%')) or coalesce(upper(function('unaccent',s.strSigle)), '') like upper(concat('%', coalesce(?2, ''), '%')) or coalesce(upper(function('unaccent',s.strTel)), '') like upper(concat('%', coalesce(?2, ''), '%')) or coalesce(upper(function('unaccent', s.strAddress)), '') like upper(concat('%', coalesce(?2, ''), '%')) or coalesce(upper(function('unaccent', s.situationGeo)), '') like upper(concat('%', coalesce(?2, ''), '%')) or upper(function('unaccent',s.strType.name)) like upper(concat('%', coalesce(?2, ''), '%'))) and s.status = ?3 order by s.strName")
    ...
    @Query("select s from Structure s where s.strType.uniqueCode in (select tp.parent.uniqueCode from TypeParam tp where tp.child.uniqueCode = ?1 and tp.status = 'ACTIVE') and (s.strId = ?2 or locate(concat(function('getStrCode', ?2), '/'), s.strCode) = 1)  and s.status = 'ACTIVE'")
    ...
    @Query("select s from Structure s where s.strType.uniqueCode in (select tp.parent.uniqueCode from TypeParam tp where tp.child.uniqueCode = function('getStrTypeCode', ?1) and tp.status = 'ACTIVE') and (s.strId = ?2 or locate(concat(function('getStrCode', ?2), '/'), s.strCode) = 1)  and s.status = 'ACTIVE'")
    ...
    @Query("select new rigeldevsolutions.gestasso.structuremodule.model.dtos.ChangeAnchorDTO(s.strId, s.strType.uniqueCode, s.strParent.strId, s.strName, s.strSigle, s.strLevel) from Structure s where s.strId = ?1")
    ...
    @Query("select new rigeldevsolutions.gestasso.sharedmodule.dtos.SelectOption(s.strId, s.strName, 'function(get_hierarchy_sigles,s.strId)') from Structure s")
